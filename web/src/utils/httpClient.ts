...
export const customFetch = <T>(url: string, options: RequestInit = {}): FetchResponse<T> => {
  // Cancel logic
  const controller = new AbortController();

  // Inject default headers
  const headers = options ? { ...defaultHeaders, ...options.headers } : defaultHeaders;
  ...
  // handle result
  const returnPromise: Promise<T> = responsePromise.then(async (response: Response) => {
    if (response.status < 300) {
      ...
      switch (response.status) {
        ...
        case 401:
          throw new GenericRequestError(HttpErrors.UnauthenticatedRequestError);
        case 403:
          throw new GenericRequestError(HttpErrors.ForbiddenRequestError);
        case 404:
          throw new GenericRequestError(HttpErrors.NotFoundRequestError);
        default:
          throw new GenericRequestError(HttpErrors.UnknownRequestError, await response.json());
      }
    }
  });
