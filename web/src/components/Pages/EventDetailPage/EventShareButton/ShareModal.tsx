...
const ShareModal: React.FC<Props> = ({ className, isOpen, onOpen, content, onClose, type }) => {
  const { t } = useTranslation();
  let { href } = useLocation();

  const firstIndex = href?.indexOf('/checkout');
  href = firstIndex > -1 ? href?.slice(0, firstIndex) : href;
  const [isCopied, setIsCopied] = useState(false);

  const shareContent = content
    ? content
    : `Check out this ${type ? type : 'event'} from Festival Pass!`;
      ...
      <Modal show={isOpen} onHide={onClose} className="animated bounceIn">
        ...
        <ModalBody>
          <div style={{ textAlign: 'left' }}>
            <S.ShareModalHeading>{`Share this ${
              type ? type : 'event'
            } with friends`}</S.ShareModalHeading>
