...
const EventsImageGallery: React.FC<Props> = ({
  className,
  ...
  const data = useStaticQuery<GatsbyEventImageGalleryQuery>(eventsImageGalleryQuery);
    ...
    <div className={className}>
      {images.length > 4 ? (
        <div>
          ...
          <S.GalleryWrapperSmall className="row">
            <S.GalleryContainerSmall className="">
              {video_url && <PlayYoutubeVideoButton youtubeVideoUrl={video_url} />}
              <S.ClickableWrapper onClick={() => selectImage(0)}>
                <S.CoverImagesmall multiple {...getImageProps(head(images))} />
              </S.ClickableWrapper>
              {images.map((img, idx: number) => {
                if (idx > 0) {
                  return (
                    <a key={idx} onClick={() => selectImage(idx)}>
                      <S.GalleryImageSmall {...getImageProps(img)} />
                    </a>
                  );
                }
                return null;
              })}
            </S.GalleryContainerSmall>
          </S.GalleryWrapperSmall>
        </div>
      ) : (
        <S.RelativeContainer>
          {video_url && <PlayYoutubeVideoButton youtubeVideoUrl={video_url} />}
...
const eventsImageGalleryQuery = graphql`
  query EventsImageGallery {
